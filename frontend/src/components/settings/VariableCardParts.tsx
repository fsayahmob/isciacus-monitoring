...
export function VariableBadges({
  ...
  return (
    <>
      {isRequired && (
        <span className="rounded bg-red-100 px-1.5 py-0.5 text-xs font-medium text-red-700">Requis</span>
      )}
      {isSecret && (
        <span className="rounded bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700">Secret</span>
      )}
      {hasChanges && (
        <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-700">Modifié</span>
      )}
    </>
  )
}
  ...
  return (
    <>
      <button
        ...
        onClick={onToggle}
      >
        <svg className={`h-4 w-4 transition-transform ${showHelp ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
